...
// these after `#[host_module]` (same file, source order) so the declaration exists when
// the modules are checked. Every module the binary reaches has to be here: `preload` is
// the only searcher an embedded run has, and a `require` of a name nobody registered fails
// at the line that needs it rather than at startup.
...
/// `root`, then the Teal modules as `require("cardbox.cards")` and `require("cardbox")`.
pub fn preload(h: &Htl, root: &Path) -> anyhow::Result<()> {
    ...
    //
    // `cardbox.cards` before `cardbox`, because `cardbox` requires it as it loads. Lua
    // only consults `package.preload` when the `require` runs, so the order is not what
    // makes this work — it is what keeps the file honest about the dependency.
...
mod tests {
    ...
    /// has to add all of it back up out of the events alone.
    #[test]
