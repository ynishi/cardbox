...
//!
//! [`CardsProjection::NAME`] is `cards_v1`, and the suffix is the rebuild convention
//! rather than decoration: a projection's name **is** the primary key of its checkpoint,
//! so changing the shape of these tables in a way that old rows cannot be migrated into is
//! done by renaming the projection to `cards_v2`. The new name has no checkpoint, starts
//! at the beginning of the log and builds its own tables, and the old model stays readable
//! until the switch is made. Calling `rebuild()` on the same name is the other route: it
//! empties the model and replays, which is right when the shape did not change and the
//! fold did.
//!
//! # The `cb_` prefix
//!
...
/// The read model over a card's five kinds.
///
/// Stateless: everything it knows is in the tables, which is what makes a rebuild a replay
/// rather than a reconstruction of anything held here.
...
impl CardsProjection {
    ...
    /// the `_v1` is for.
    pub const NAME: &'static str = "cards_v1";

    /// The kinds a card's stream carries. Naming them is not only a filter: it is what
    /// lets the runner read through the `(kind, position)` index instead of walking the
    /// whole log, and it is the reason `apply` may treat an unknown kind as a bug.
...
impl Projection for CardsProjection {
    ...
    fn apply(&mut self, tx: &Transaction<'_>, event: &eventsdb::Recorded) -> Result<()> {
        let kind = event.kind();
        let id = card_id(&event.stream, kind)?;
        ...
        match kind {
            "card_opened" => opened(tx, id, epoch_ms, position, meta, data),
            "samples_appended" => samples(tx, id, seq, epoch_ms, data),
            "eval_recorded" => eval(tx, id, seq, epoch_ms, data),
            "checkpoint_saved" => checkpoint(tx, id, seq, epoch_ms, data),
            "card_closed" => closed(tx, id, epoch_ms, meta, data),
